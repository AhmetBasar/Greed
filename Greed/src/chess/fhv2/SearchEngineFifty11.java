...
import chess.evaluation.EvaluationAdvancedV5;
...
//		System.out.println("Yüzde = % " + (100 * usedTTSize) / TT_SIZE);
	...
	private int quiescentSearch(IBoard board, int side, int color, int alpha, int beta, int depth){
		int standPatScore =  color * EvaluationAdvancedV5.evaluate(board.getBitboard(), board.getCastlingRights(depth + 1), side ^ 1, pawnHashTable);
