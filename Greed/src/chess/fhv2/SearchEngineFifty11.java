...
public class SearchEngineFifty11 implements ISearchableV2, EngineConstants {
	...
	public SearchResult search(SearchParameters searchParameters) {
		...
		for (int i = 1; isFixedDepth ? (i <= depth) : true; i++) {
			currentDepth = i;
			IBoard board = BoardFactory.getInstance6(searchParameters.getBitboard(), searchParameters.getPieces(), searchParameters.getEpT(), searchParameters.getEpS(), i, searchParameters.getCastlingRights(), searchParameters.getUiZobristKey(), searchParameters.getFiftyMoveCounter(), pawnZobristKey);
