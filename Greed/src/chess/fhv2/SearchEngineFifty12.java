...
import chess.evaluation.EvaluationAdvancedV6;
...
public class SearchEngineFifty12 implements ISearchableV2, EngineConstants {
	...
	private static SearchEngineFifty12 instance;
	private static SearchEngineFifty12[] newInstances = new SearchEngineFifty12[16];
	
	private SearchEngineFifty12(){
		TranspositionTable.fillZobristArrays();
		timeControllerThread.start();
	}
	
	public static SearchEngineFifty12 getInstance() {
		if (instance == null) {
			instance = new SearchEngineFifty12();
		}
		return instance;
	}
	
	public static synchronized SearchEngineFifty12 getNewInstance() {
		for (int i = 0; i < newInstances.length; i++) {
			if (newInstances[i] == null) {
				newInstances[i] = new SearchEngineFifty12();
...
//			IBoard board = BoardFactory.getInstance(searchParameters.getBitboard(), searchParameters.getPieces(), searchParameters.getEpT(), searchParameters.getEpS(), i, searchParameters.getCastlingRights(), searchParameters.getUiZobristKey(), searchParameters.getFiftyMoveCounter());
			IBoard board = new BoardV6(searchParameters.getBitboard(), searchParameters.getPieces(), searchParameters.getEpT(), searchParameters.getEpS(), i, searchParameters.getCastlingRights(), searchParameters.getUiZobristKey(), searchParameters.getFiftyMoveCounter(), pawnZobristKey);
...
//		System.out.println("Yüzde = % " + (100 * usedTTSize) / TT_SIZE);
	...
	private int quiescentSearch(IBoard board, int side, int color, int alpha, int beta, int depth){
		int standPatScore =  color * EvaluationAdvancedV6.evaluate(board.getBitboard(), board.getCastlingRights(depth + 1), side ^ 1, board.getPawnZobristKey(depth + 1), pawnHashTable);
