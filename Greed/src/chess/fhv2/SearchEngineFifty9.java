...
import chess.engine.ITranspositionTable;
...
public class SearchEngineFifty9 implements ISearchableV2, EngineConstants {
	...
	private static SearchEngineFifty9 instance;
	private static SearchEngineFifty9[] newInstances = new SearchEngineFifty9[16];
	
	private SearchEngineFifty9(){
		TranspositionTable.fillZobristArrays();
		timeControllerThread.start();
	}
	
	public static SearchEngineFifty9 getInstance() {
		if (instance == null) {
			instance = new SearchEngineFifty9();
		}
		return instance;
	}
	
	public static synchronized SearchEngineFifty9 getNewInstance() {
		for (int i = 0; i < newInstances.length; i++) {
			if (newInstances[i] == null) {
				newInstances[i] = new SearchEngineFifty9();
	...
	private ITranspositionTable tt = new TTHybrid();
